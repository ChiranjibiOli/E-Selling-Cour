...
if ($studentId <= 0 || ($courseId <= 0 && ($slug === '' || preg_match('/^[a-z0-9]+(?:-[a-z0-9]+)*$/', $slug) !== 1))) {
...
try {
    ...
    Auth::redirect('checkout.php?buy_now=1');
    ...
    error_log('Buy-now failed: ' . $exception->getMessage());
    Auth::redirect('student-browse-courses.php?buy_error=1');
}
