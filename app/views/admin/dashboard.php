...
<style>
.command-page{min-height:calc(100vh - 72px);padding:34px 18px 68px}.command-shell{width:min(1320px,100%);margin:auto}.command-hero{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:22px;align-items:center;margin-bottom:20px;padding:30px;border-radius:26px;color:#fff;background:linear-gradient(135deg,#0f172a,#1e293b 55%,#312e81);box-shadow:0 24px 65px rgba(15,23,42,.2)}.command-hero h1{margin:6px 0 8px;color:#fff;font-size:clamp(2rem,4vw,3.2rem);letter-spacing:-.05em}.command-hero p{max-width:720px;margin:0;color:#cbd5e1}.command-kicker{color:#a5b4fc!important;font-size:.72rem;font-weight:900;letter-spacing:.14em;text-transform:uppercase}.hero-alerts{display:grid;grid-template-columns:repeat(3,110px);gap:10px}.hero-alerts a{display:grid;place-items:center;min-height:90px;border:1px solid rgba(255,255,255,.13);border-radius:17px;color:#fff;background:rgba(255,255,255,.08);text-decoration:none;text-align:center}.hero-alerts strong{display:block;font-size:1.45rem}.hero-alerts span{font-size:.67rem;font-weight:800}.metric-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-bottom:20px}.command-metric{position:relative;overflow:hidden;padding:19px;border:1px solid #e5eaf1;border-radius:19px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.055)}.command-metric::after{content:'';position:absolute;right:-18px;bottom:-26px;width:80px;height:80px;border-radius:999px;background:var(--metric-color,#eef2ff)}.command-metric span{color:#667085;font-size:.72rem;font-weight:850;text-transform:uppercase}.command-metric strong{display:block;margin-top:7px;color:#101828;font-size:1.75rem;letter-spacing:-.04em}.command-metric small{color:#98a2b3}.command-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.command-panel{overflow:hidden;border:1px solid #e5eaf1;border-radius:21px;background:#fff;box-shadow:0 11px 30px rgba(15,23,42,.06)}.panel-heading{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:17px 18px;border-bottom:1px solid #edf0f5}.panel-heading h2{margin:0;color:#101828;font-size:1rem}.panel-heading a{color:#4f46e5;text-decoration:none;font-size:.72rem;font-weight:900}.activity-list{display:grid}.activity-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 18px;border-bottom:1px solid #eef2f6;text-decoration:none}.activity-row:last-child{border-bottom:0}.activity-row:hover{background:#fafbff}.activity-row strong{display:block;color:#344054;font-size:.82rem}.activity-row span{display:block;margin-top:3px;color:#98a2b3;font-size:.68rem}.activity-badge{flex:0 0 auto!important;margin:0!important;padding:6px 8px;border-radius:999px;color:#475467!important;background:#f1f5f9;font-size:.66rem!important;font-weight:850}.empty-activity{padding:28px 18px;color:#98a2b3;text-align:center;font-size:.82rem}.quick-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:20px}.quick-grid a{padding:17px;border:1px solid #e5eaf1;border-radius:17px;color:#344054;background:#fff;text-decoration:none;font-size:.78rem;font-weight:900;box-shadow:0 8px 22px rgba(15,23,42,.04);transition:.18s}.quick-grid a:hover{transform:translateY(-2px);border-color:#c7d2fe;color:#4338ca}@media(max-width:1050px){.metric-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.command-grid{grid-template-columns:1fr 1fr}.command-panel:last-child{grid-column:1/-1}.quick-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:720px){.command-page{padding:20px 12px 48px}.command-hero{grid-template-columns:1fr;padding:22px}.hero-alerts{grid-template-columns:repeat(3,minmax(0,1fr))}.command-grid{grid-template-columns:1fr}.command-panel:last-child{grid-column:auto}.metric-grid,.quick-grid{grid-template-columns:1fr 1fr}}@media(max-width:430px){.hero-alerts,.metric-grid,.quick-grid{grid-template-columns:1fr}}
</style>
    ...
    <section class="command-shell">
        <header class="command-hero">
            <div>
                ...
                <p>Review only the items that need attention, then move directly into courses, payments, instructors, or payouts.</p>
            </div>
        ...
        <div class="metric-grid">
            <article class="command-metric" style="--metric-color:#e0e7ff"><span>Active students</span><strong><?php echo $metrics['students']; ?></strong><small>Learning accounts</small></article>
            <article class="command-metric" style="--metric-color:#dbeafe"><span>Active instructors</span><strong><?php echo $metrics['instructors']; ?></strong><small><?php echo $metrics['instructor_queue']; ?> applications waiting</small></article>
            <article class="command-metric" style="--metric-color:#d1fae5"><span>Published courses</span><strong><?php echo $metrics['published_courses']; ?></strong><small>Available in marketplace</small></article>
            <article class="command-metric" style="--metric-color:#fef3c7"><span>Verified revenue</span><strong>Rs. <?php echo number_format($metrics['revenue'], 0); ?></strong><small>Paid transactions only</small></article>
        </div>
            ...
            <section class="command-panel">
                ...
                <div class="activity-list">
                    <?php foreach ($pendingCourses as $course): ?>
                        <a class="activity-row" href="course-details.php?slug=<?php echo rawurlencode($course['slug']); ?>">
                ...
                <div class="activity-list">
                    <?php foreach ($recentOrders as $order): ?>
                        <a class="activity-row" href="admin-order-details.php?order_id=<?php echo (int) $order['id']; ?>">
                            ...
                            <span class="activity-badge"><?php echo admin_dashboard_h(ucfirst($order['order_status'])); ?></span>
                        </a>
                    <?php endforeach; ?>
                </div>
            </section>
