...
final class StudentMyCoursesPage
{
    public static function render(array $courses, string $error = ''): Response
    {
        ...
        foreach ($courses as $course) {
            ...
            $totalLessons = max(0, (int) ($course['total_lessons'] ?? $course['lesson_count'] ?? 0));
            $completedLessons = max(0, min($totalLessons, (int) ($course['completed_lessons'] ?? 0)));
            $progressPercent = max(0, min(100, (int) ($course['progress_percent'] ?? 0)));
            $isCompleted = (bool) ($course['is_completed'] ?? ($totalLessons > 0 && $completedLessons === $totalLessons));
            ...
            $badgeClass = $isCompleted ? 'completed' : $status;
            $badgeLabel = $isCompleted ? 'Completed' : ucfirst($status);
            $learningAction = $isCompleted ? 'Review course' : ($completedLessons > 0 ? 'Continue learning' : 'Start learning');
            $lessonSummary = $totalLessons > 0
                ? $completedLessons . ' of ' . $totalLessons . ' lessons complete'
                : 'Curriculum is being prepared';

            $cards .= '<article class="learning-course-card"><div class="learning-course-media">' . $media . '<span class="status-badge ' . $e($badgeClass) . '">' . $e($badgeLabel) . '</span></div>'
