...
final class LandingCourseCard
{
    public static function render(array $course): string
    {
        ...
        $imageHtml = $image !== ''
            ...
            : '<div class="course-card-placeholder">CourseHub</div>';
        $price = (float) ($course['price'] ?? 0);
        $priceText = $price > 0 ? 'NPR ' . number_format($price, 0) : 'Free';

        return '<article class="course-card"><a class="course-card-media" href="/course?id=' . $id . '">' . $imageHtml . '</a>'
            ...
            . '<footer><small>By ' . $e($course['instructor_name'] ?? 'CourseHub instructor') . '</small><strong>' . $e($priceText) . '</strong></footer></div></article>';
    }
}
