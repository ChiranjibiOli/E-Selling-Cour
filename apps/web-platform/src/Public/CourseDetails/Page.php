...
final class CourseDetailsPage
{
    public static function render(CourseDetailsViewModel $model): Response
    {
        ...
        if ($curriculum === '') {
            $curriculum = '<div class="detail-empty">Curriculum will be added by the course creator.</div>';
        }

        $list = static function (mixed $items, callable $escape): string {
            if (!is_array($items) || $items === []) {
                return '<li>Details will be added soon.</li>';
            }
            $html = '';
        ...
        $ownershipChecked = (bool) ($course['ownership_checked'] ?? ($viewerRole !== 'student'));

        if ($owned) {
            $action = '<a class="buy-button" href="/student/course-player?course=' . (int) $course['id'] . '">Continue learning</a>';
            $priceHtml = '<strong class="price">Already purchased</strong>';
        } elseif ($viewerRole === 'student' && !$ownershipChecked) {
            $action = '<a class="buy-button" href="/student/my-courses">Check My Courses</a>';
            $priceHtml = '<strong class="price">Ownership check required</strong>';
        } elseif ($viewerRole === 'student') {
            $action = $price > 0
                ? '<a class="buy-button" href="/student/cart?add=' . (int) $course['id'] . '">Buy once · NPR ' . number_format($price, 0) . '</a>'
                : '<a class="buy-button" href="/student/cart?add=' . (int) $course['id'] . '">Enrol free</a>';
        } else {
            $action = '<a class="buy-button" href="/learn/sign-in">Sign in to enrol</a>';
        }
        $accountState = $viewerRole === 'student' ? 'student' : 'guest';

        $html = '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#f7f0e5"><title>' . $e($course['title'] ?? 'Course') . ' | CourseHub</title>'
            ...
            . '<link rel="stylesheet" href="/room-assets/Public/CourseDetails/page.css"><link rel="stylesheet" href="/assets/css/public-site.css?v=20260728-1"><link rel="stylesheet" href="/assets/css/course-human-system.css?v=20260728-1">' . PublicNavbar::styles() . '</head><body class="detail-body">'
            . PublicNavbar::render('courses', $accountState) . '<main>'
            . '<section class="detail-hero"><div class="detail-copy"><a href="/courses">← Back to courses</a><span>' . $e($course['category_name'] ?? 'Course') . '</span><h1>' . $e($course['title'] ?? 'Untitled course') . '</h1>'
            . ($subtitle !== '' ? '<h2>' . $e($subtitle) . '</h2>' : '') . '<p>' . $e($course['short_description'] ?? '') . '</p>' . $introLink
            . '<div class="detail-meta"><div><small>Course creator</small><strong>' . $e($course['instructor_name'] ?? 'CourseHub creator') . '</strong></div><div><small>Level</small><strong>' . $e(ucfirst((string) ($course['level'] ?? 'beginner'))) . '</strong></div><div><small>Language</small><strong>' . $e($course['language'] ?? 'English') . '</strong></div></div></div>'
            . '<aside class="detail-purchase"><div class="detail-image">' . $imageHtml . '</div><div class="purchase-body">' . $priceHtml . $action . '<ul><li>Lifetime course access</li><li>' . $lessonCount . ' structured lessons</li><li>' . $previewCount . ' public previews</li><li>Progress tracking</li><li>Verified gateway or manual payment</li></ul></div></aside></section>'
            ...
            . '<section class="curriculum"><div class="curriculum-heading"><span>CURRICULUM</span><h2>Course structure</h2><p>Preview lessons are available before enrolment. Full lessons unlock after a verified payment.</p></div><div class="curriculum-list">' . $curriculum . '</div></section>'
            . '</main><footer class="detail-footer"><a href="/">CourseHub</a><span>Education that moves with you.</span></footer>' . PublicNavbar::script() . '<script src="/assets/js/course-card-theme.js" defer></script></body></html>';
