...
$server = $conn->query(
    'SELECT DATABASE() AS database_name, @@port AS server_port, VERSION() AS server_version'
)->fetch_assoc();
...
$requiredSchema = [
    ...
    'categories' => ['id', 'name', 'slug', 'description', 'status', 'is_active', 'created_at'],
...
$healthy = $databaseName === 'coursehub'
    && $serverPort === 3307
    && $missingTables === []
    && $missingColumns === [];
...
echo 'Database: ' . $databaseName . ($databaseName === 'coursehub' ? ' [OK]' : ' [EXPECTED coursehub]') . PHP_EOL;
...
if ($healthy) {
    echo 'RESULT: PASS - coursehub on port 3307 is ready.' . PHP_EOL;
    exit(0);
}

echo 'RESULT: FAIL - apply database/migrations/20260712_coursehub_compatibility.sql and run this command again.' . PHP_EOL;
exit(1);
